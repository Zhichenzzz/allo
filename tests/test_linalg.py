...
def test_library_higher_dimension_ops(enable_tensor):
    ...
    B = np.random.uniform(size=(L, K)).astype(np.float32)
    C = np.random.uniform(size=(L,)).astype(np.float32)

    def kernel(A: float32[M, K, L], B: float32[L, K], C: float32[L]) -> float32[M, N]:
        output1 = allo.transpose(A, (-1, -2))
        output2 = allo.linear(output1, B, C)
        output = allo.view(output2, (5, 4))
        return output
