...
class GPTneo(nn.Module):
    ...
    def forward(self, x, kcache=None, vcache=None, n=0):
        ...
        m = n + 5
        tmp = torch.zeros((1, 12, 14 - n, 64))
        for i in range(self.layers):
            block = self.transformer[i]
            x, updated_kcache, updated_vcache = block(x, kcache[i], vcache[i], m, tmp)
...
class TransformerBlock(nn.Module):
    ...
    def forward(self, x, kcache=None, vcache=None, m=0, tmp=None):
        ln_1 = self.ln_1(x)
        attn_output, kcache_updated, vcache_updated = self.attn(
            ln_1, kcache, vcache, m, tmp
        )
        out1 = x + attn_output
...
class MultiHeadAttention(nn.Module):
    ...
    def forward(self, x, kcache=None, vcache=None, m=0, tmp=None):
        ...
        if kcache is not None:
            ...
            old_k = old_k[:, :, :m, :]
            # old_k = torch.narrow(old_k, dim=-2, start=0, length=n + 4)
            old_v = old_v[:, :, :m, :]
            # old_v = torch.narrow(old_v, dim=-2, start=0, length=n + 4)
            k = torch.cat([old_k, new_k], dim=-2)
            v = torch.cat([old_v, new_v], dim=-2)

        new_k_cache = torch.cat([k, tmp], dim=-2)
        new_v_cache = torch.cat([v, tmp], dim=-2)
    ...
    with torch.no_grad():
        ...
        for num in tqdm(
            ...
            if kcache is None:
                ...
                llvm_mod = allo.frontend.from_pytorch(
                    model,
                    example_inputs=[x, kcache, vcache, num],
                    verbose=True,
                )
                x = x.detach().numpy()
