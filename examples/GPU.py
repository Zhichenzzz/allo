...
class GPTneo(nn.Module):
    ...
    def forward(self, x, kvcache=None, n=0):
        if kvcache is None:
            kvcache = [None] * len(self.transformer)
        presents = () if kvcache is not None else None
        for i in range(self.layers):
            block = self.transformer[i]
            kvcache_block = kvcache[i]
            x, updated_cache = block(x, kvcache[i], n)
...
class TransformerBlock(nn.Module):
    ...
    def forward(self, x, kvcache=None, n=0):
        ln_1 = self.ln_1(x)
        attn_output, kvcache_updated = self.attn(ln_1, kvcache, n)
        out1 = x + attn_output
...
class MultiHeadAttention(nn.Module):
    ...
    def forward(self, x, kvcache=None, n=0):
        ...
        shape = k.shape[:2] + (25, k.shape[-1])
        print(shape)
        k_cache = torch.cat([k, torch.zeros((1, 12, 14 - n, 64))], dim=-2)
        v_cache = torch.cat([v, torch.zeros((1, 12, 14 - n, 64))], dim=-2)
        current_cache = [k_cache, v_cache]
...
def generate(inputs, model, embeddings, n_tokens_to_generate):
    with torch.no_grad():
        ...
        for _ in tqdm(
            ...
            logits, kvcache = model(x, kvcache, _)  # model forward pass
